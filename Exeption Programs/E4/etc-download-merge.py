"""
E4 — Download ETC files from submissions DB and merge selected columns.

1. Read DB settings from .env in this folder.
2. Query rows for ENTITY_NAME + date interval; URL column = etc_file_url.
3. Download each file into DOWNLOAD_FOLDER.
4. Detect header via keywords in etc_merge_config.json.
5. Map alternate column names → canonical merge columns and concat.
6. After a successful merge write, delete the downloaded ETC files.

Edit runtime inputs below, then:
  python etc-download-merge.py
"""
...
CONFIG_PATH = BASE_DIR / "etc_merge_config.json"

# --- Runtime inputs (edit these; not in JSON) ---
ENTITY_NAME = "odhaki_paipkhar"
FROM_DATE = "2026-01-01"  # inclusive YYYY-MM-DD
TO_DATE = "2026-04-30"  # inclusive YYYY-MM-DD

DOWNLOAD_FOLDER = BASE_DIR / "etc_downloads"
# Built at run time as: {entity_name}_{FROM_DATE}_{TO_DATE}_merged_etc.csv
MERGED_OUTPUT_DIR = BASE_DIR / "output"

DOWNLOAD_TIMEOUT_SECONDS = 180
SKIP_EXISTING = True
# If True: only download, skip merge step.
DOWNLOAD_ONLY = False
...
def detect_header_row(
    ...
    *,
    ...
    if not keyword_keys:
        raise ValueError("header_keywords is empty in etc_merge_config.json")
...
def resolve_column(headers: list[str], aliases: list[str]) -> str | None:
    """Return the first header that matches any alias (case/space-insensitive)."""
...
def extract_merge_frame(
    ...
    *,
    ...
    if not merge_columns:
        raise ValueError("merge_columns is empty in etc_merge_config.json")
...
def list_local_etc_files(folder: Path) -> list[Path]:
...
def merge_etc_files(paths: list[Path], config: dict, output_path: Path) -> Path:
    ...
    if not paths:
        raise FileNotFoundError("No ETC files to merge.")
    ...
    if not frames:
        raise RuntimeError(
            "No ETC files could be parsed for merge.\n" + "\n".join(errors)
        )

    merged = pd.concat(frames, ignore_index=True)
    # Drop rows that are entirely blank across merge columns
    ...
    # CSV avoids Excel's ~1,048,576 row limit on large ETC merges.
...
def delete_downloaded_files(paths: list[Path], download_folder: Path) -> int:
    """Remove merged source downloads and prune empty directories under download_folder."""
    deleted = 0
    for path in paths:
        try:
            if path.is_file():
                path.unlink()
                deleted += 1
                print(f"Deleted download: {path}")
        except OSError as exc:
            print(f"WARNING could not delete {path}: {exc}")

    # Remove empty date/shift folders bottom-up; keep DOWNLOAD_FOLDER itself.
    ...
    print(f"Deleted {deleted} downloaded ETC file(s).")
    return deleted
...
def run_etc_download_merge(
    ...
    *,
    ...
    with psycopg2.connect(**connection_kwargs()) as conn:
        local_paths, _stats = download_etc_files(
            conn,
            entity_name=name,
            start=start,
            end=end,
    ...
    if download_only:
        print("\nDOWNLOAD_ONLY=True — merge skipped.")
        return None

    paths = local_paths or list_local_etc_files(out_download / safe_part(name))
    if not paths:
        paths = list_local_etc_files(out_download)
    if not paths:
        raise FileNotFoundError(
            f"No ETC files downloaded for entity={name!r} "
            f"range {start.isoformat()} → {end.isoformat()}"
        )

    merged_name = (
        f"{safe_part(name)}_{start.isoformat()}_{end.isoformat()}_merged_etc.csv"
    )
...
def main() -> int:
    run_etc_download_merge(ENTITY_NAME, FROM_DATE, TO_DATE)
    return 0
