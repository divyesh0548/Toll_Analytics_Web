"""
Updates "exempt_distribution_per_lane" with hourly exempt vehicle counts by lane.

One row per (plaza, date, hour); columns l01…l12 hold exempt counts for that lane.
Does not modify existing analytics / gap tables.
"""
...
EXEMPT_MOP_BUCKET = "exempt"

DATA_COLUMNS = ["plaza_name", "date", "hour", *LANE_COLUMNS.values()]
...
def prepare_dataframe(df: pd.DataFrame, datetime_format: str) -> pd.DataFrame:
    ...
    prepared = prepared[
        prepared["event_dt"].notna()
        & prepared["lane_no"].notna()
        & prepared["mop"].eq(EXEMPT_MOP_BUCKET)
    ].copy()
...
def aggregate_hourly_exempt_rows(df: pd.DataFrame) -> list[dict]:
    """One row per plaza + date + hour with exempt counts in l01…l12."""
    rows: list[dict] = []
    if df.empty:
        return rows
...
def print_hourly_rows(rows: list[dict]) -> None:
    if not rows:
        print("  No hourly exempt rows prepared.")
        return
...
def fetch_existing_hour_keys(conn, table_name: str, rows: list[dict]) -> set[tuple]:
    ...
    keys = [(row["plaza_name"], row["date"], row["hour"]) for row in rows]
    query = sql.SQL(
        "SELECT plaza_name, date, hour FROM {table} "
        "WHERE (plaza_name, date, hour) IN %s"
    ).format(table=sql.Identifier(table_name))

    with conn.cursor() as cursor:
        cursor.execute(query.as_string(conn), (tuple(keys),))
        return {(plaza_name, record_date, hour) for plaza_name, record_date, hour in cursor.fetchall()}
...
def upsert_rows(conn, table_name: str, rows: list[dict]) -> tuple[int, int]:
    ...
    for row in rows:
        key = (row["plaza_name"], row["date"], row["hour"])
        if key in existing_keys:
            print(
                f"  SKIP (exists): plaza={row['plaza_name']}, "
                f"date={row['date']}, hour={row['hour']}"
            )
            skipped += 1
        else:
...
def process_file(
    ...
    if prepared.empty:
        print(
            f"  No exempt rows on or after {START_DATE_LIMIT} "
            "(or no mapped exempt+lane rows). Skipping file."
        )
        return 0, False

    hourly_rows = aggregate_hourly_exempt_rows(prepared)
    ...
    print(f"  Hourly exempt rows inserted: {inserted}, skipped (already exists): {skipped}")
...
def process_folder(folder_path: str, dry_run: bool = DRY_RUN) -> None:
    ...
    table_name = EXEMPT_DISTRIBUTION_TABLE
    ...
    run_log = create_run_log_file(INSIGHTS_DIR, "module3")
    ...
    if dry_run:
        ...
        append_run_log(
            run_log,
            f"\nDry run complete. Total hourly exempt rows prepared: {total_rows}. "
    ...
    try:
        ensure_exempt_distribution_table(conn, table_name, list(LANE_COLUMNS.values()))
        for file_path in files:
            try:
                rows, file_skipped = process_file(
                    file_path,
        ...
        append_run_log(
            run_log,
            f"\nCompleted. Total hourly exempt rows inserted: {total_rows}. "
            f"Skipped files: {skipped_files}.",
        )
        print(f"\nCompleted. Total hourly exempt rows inserted: {total_rows}")
        print(f"Skipped files: {skipped_files}. Log: {run_log}")
